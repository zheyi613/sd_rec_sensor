...
static inline void spi_rx(BYTE *data, WORD size)
{
        spi_txrx(DO_H, data, size);
}
...
static int xmit_datablock(const BYTE *buff, BYTE token)
{
	...
	spi_rx(&resp, 1); /* Wait 8 cycle(Nwr = 1) before writing data */
	spi_tx(&token, 1);    /* Send start block token */
	if (token != 0xFD) {  /* Send data if token is other than Stop Tran */
                spi_tx((BYTE *)buff, 512);
		spi_tx(crc, 2); /* Transmit dummy CRC (0xFFFF) */
...
static BYTE send_cmd(BYTE cmd_id, DWORD arg)
{
        ...
        spi_tx(cmd, CMD_SIZE); /* Send command first */
        if (cmd_id == CMD12)
                spi_rx(&resp, 1);
...
DSTATUS disk_initialize(BYTE drv)
{
        ...
        if(send_cmd(CMD8, 0x1AA) == 0x01) {
                spi_rx(resp, 4); /* Check R7 following echo packet */
        ...
        /* read OCR */
        if (!send_cmd(CMD58, 0)) {
                spi_rx(resp, 4); /* Receive R3 following OCR */
...
DRESULT disk_ioctl (BYTE drv, BYTE cmd, void *buff)
{
	...
	case GET_BLOCK_SIZE:/* Get erase block size in unit of sector(DWORD) */
		if (cardtype & CT_SDC2) {	/* SDC ver 2+ */
                        ...
                        spi_rx(csd, 1);         /* Purge R2 second byte */
